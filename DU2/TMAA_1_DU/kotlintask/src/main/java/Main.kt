...
fun onlyChosen(firstArray: IntArray , secondArray: IntArray, decider : ( Int , Int ) -> Int) : IntArray {
    ...
    } //kotlin base funkce, něco jako if(cond) -> do ,else do error
...
fun String.toggle(): String {
    ...
    for (c in this) {
        toggledString += when { //lepší zápis než toggledstring = toggledsting + char
            c == ' ' -> '*'
...
fun test(text: String): Int {
    var i = 0 //mutables vs non mutables
...
interface HashTable<T> { //<T> generický typ
    val size : Int
